...
m.nmap("<leader>wh", "<C-W>h", { desc = "Got to window left" })
m.nmap("<leader>wj", "<C-W>j", { desc = "Got to window below" })
m.nmap("<leader>wk", "<C-W>k", { desc = "Got to window up" })
m.nmap("<leader>wl", "<C-W>l", { desc = "Got to window right" })
m.nmap("<leader>wq", "<C-W>q", { desc = "Quit a window" })
...
m.nmap(
  "<leader>sr",
  [[<esc>:%s/<C-R><C-W>//g<Left><Left><BackSpace>/]],
...
m.nmap(
  "<leader>sl",
  [[<esc>:%s/.*<C-R><C-W>.*//g<Left><Left><BackSpace>/]],
  { desc = "Search & replace line containing word under cursor" }
)

m.nmap(
  "<leader>scR",
  [[<esc>:%s/\C\<<C-R><C-W>\>//g<Left><Left><BackSpace>/]],
  { desc = "Search & replace WORD under cursor case sensitive" }
)
m.nmap(
  "<leader>scr",
  [[<esc>:%s/\C<C-R><C-W>//g<Left><Left><BackSpace>/]],
...
m.vmap(
  "<leader>scr",
  [[y<esc>:%s/\C<C-R>0//g<Left><Left><BackSpace>/]],
  { desc = "Search & replace selected case sensitive" }
)
m.nmap(
  "<leader>scl",
  [[<esc>:%s/\C.*<C-R><C-W>.*//g<Left><Left><BackSpace>/]],
  { desc = "Search & replace line containing word under cursor case sensitive" }
)
...
m.vmap(
  "<leader>y",
  "\"+y<cmd>lua require('utils').notify_info('Mappings', 'The selected text was copied to the clipboard')<cr>",
  { desc = "Copy selected text to the clipboard" }
)
m.nmap(
  "<leader>Y",
  "\"+yy<cmd>lua require('utils').notify_info('Mappings', 'The active line was copied to the clipboard')<cr>",
  { desc = "Copy selected text to the clipboard" }
)
...
m.nmap("<leader>fs", function()
  MyFoldSyntax()
end, { desc = "Set fold method syntax" })
